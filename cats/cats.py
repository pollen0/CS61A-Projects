...
def about(subject):
    ...
    # BEGIN PROBLEM 2
    def help(words):
        goodwords = split(lower(remove_punctuation(words)))
        for words in goodwords:
            for subjectword in subject:
                if words == subjectword:
                    return True
        return False
    return help
    # END PROBLEM 2
...
def wpm(typed, elapsed):
    ...
    # BEGIN PROBLEM 4
    return len(typed)/5*60.0/float(elapsed)
    # END PROBLEM 4
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        return typed_word
    closet_word = min(word_list, key = lambda word: diff_function(typed_word, word, limit))
    if diff_function(typed_word, closet_word, limit) <= limit:
        return closet_word
    return typed_word
    # END PROBLEM 5
...
def feline_fixes(typed, source, limit):
    ...
    #iterate through character by character of shorter word
    #check difference of length
    #check if less than big_limit
...
def report_progress(typed, prompt, user_id, upload):
    ...
    for i in range(len(typed)):
        if typed[i] == prompt[i]:
            ...
            done = {"id": user_id, "progress": progress}
            upload(done)
            return progress
    progress = count / len(prompt)
    done = {"id": user_id, "progress": progress}
...
def fastest_words(match):
    ...
    for words in word_indices:
        player_list = []
        lowest_time = time(match, 0, words)
        for player in player_indices:
            if time(match, player, words) < lowest_time:
                lowest_time = time(match, player, words)
                return player
        new_list[player_list] += get_word[match, words]
    return new_list
