...
def scheme_eval(expr, env, _=None):  # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in scheme_forms.SPECIAL_FORMS:
        ...
        eval_operand = rest.map(lambda x: scheme_eval(x, env)) # use map to evaluate the operands individually 
        return scheme_apply(eval_operator, eval_operand, env) #apply operators to operand
        # END PROBLEM 3
